...
jqm.update<-function(db,N,beta0,beta1,beta2,beta3,beta4,u,z,lambda.u,lambda.z,alpha) {
  tau1 = alpha/2
  tau2 = 1-tau1
  ...
  for(i in 1:N) {
    ...
    y<-db$y[db$subject==subjects[i]]-beta0-u[i]-beta3*age-beta4*sex
    db$y2[db$subject==subjects[i]]<-db$y[db$subject==subjects[i]]-beta0-u[i]-beta3*age-beta4*sex
  ...
  for(i in 1:N) {
    ...
    y<-db$y[db$subject==subjects[i]]-beta0-beta3*age-beta4*sex
    z.i<-z[i]
  ...
  # y<-db$y-beta0-db$u-beta4*db$sex
  ...
  # y<-db$y-beta0-db$u-beta3*db$age
  # w.seq<-seq(0.1,10,length.out = 20)
  # obj<-sapply(w.seq,function(x) {
...
jqm.est.fixed<-function(db,lambda.u,lambda.z,alpha) {
  ...
  beta0<-median(db$y)
  u<-numeric(N)
  fit.lqmm<-lqmm(fixed=y ~ age + sex, random=~ 1, group=subject, tau=0.5,
  ...
  beta4<-fit.lqmm$theta_x[3]
  #u<-ranef(fit.lqmm)[,1]
  
  # estimation of u
  db$y2<-db$y-beta0-beta3*db$age-beta4*db$sex
  ...
  # initial estimate of beta1 and beta2
  db$y4<-db$y-beta0-db$u-beta3*db$age-beta4*db$sex
...
jqm.coverage.new.subject<-function(res,y,y.s,age,sex,age.s,sex.s,alpha) {
  ...
  y.i<-y-res$beta0-res$beta3*age-res$beta4*sex
  ...
  coverage2<-mean((y.s>res$beta0+u.i+z.i*res$beta1+res$beta3*age.s+res$beta4*sex.s)&
                    (y.s<res$beta0+u.i+z.i*res$beta2+res$beta3*age.s+res$beta4*sex.s))
    ...
    for(l.z in lambda.z.seq) {
      ...
      for(l.u in lambda.u.seq) {
        ...
        for(s in subjects) {
          ...
          coverage[cnt]<-((y>res$beta0+res$u[cnt]+res$z[cnt]*res$beta1+res$beta3*age+res$beta4*sex)&
                            (y<res$beta0+res$u[cnt]+res$z[cnt]*res$beta2+res$beta3*age+res$beta4*sex))
          cnt<-cnt+1
        }
